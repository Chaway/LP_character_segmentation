...
- **单次分割**
  ...
  在终端执行**程序**，***filename ***是图像文件的文件名（程序文件和待分割图像都在当前目录下，否则需要指明路径）
...
- **批量分割**
  ...
  ***bash*** 脚本 ***run.sh*** 可以对目录下的所有 ****.jpg*** 文件进行分割（若要分割其它格式的图像，只需将 ***run.sh*** 中的 “jpg” 改成对应的格式后缀），批量分割时可以将__ *segment.cpp* __中显示分割结果的部分代码注释掉
